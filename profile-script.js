...
function checkRateLimit(type) {
    ...
    if (diff < RATE_LIMITS[type]) {
        ...
        let msg = `${label.charAt(0).toUpperCase() + label.slice(1)} ${limit} içinde sadece 1 kez değiştirebilirsiniz.\n\n`;
        if (days > 0) msg += `Kalan süre: ${days} gün ${hours} saat`;
        else msg += `Kalan süre: ${hours} saat`;
